...
function toSvg(node, options) {
    ...
    return Promise.resolve(node)
        ...
        .then(node => embedFonts(node, options?.fonts || []))
        .then(inlineImages)
        .then(applyOptions)
        .then(makeSvgDataUri)
...
function embedFonts(node, fonts = []) {
    return fontFaces.resolveAll(fonts).then(function (cssText) {
        if (cssText !== '') {
...
function newFontFaces() {
    ...
    function resolveAll(optionFonts = []) {
        return readAll(optionFonts)
    ...
    function readAll(optionFonts = []) {
        const css = util.asArray(document.styleSheets);
        return Promise.resolve(css)
            .then(getCssRules)
            .then(rules => selectWebFontRules(rules, optionFonts))
            .then(function (rules) {
                const res = rules.map(newWebFont);
                return res;
            });

        function findUsedFonts(rule, fonts = []) {
            const fontName = rule.cssText.match(/font-family:\s?([a-z_]+);/)?.[1];
            return fontName && fonts.includes(fontName);
        }

        function selectWebFontRules(cssRules, optionFonts = []) {
            return cssRules
                .filter(function (rule) {
                    return rule.type === CSSRule.FONT_FACE_RULE && findUsedFonts(rule, optionFonts);
                })
                .filter(function (rule) {
